...
export const magnitude = new RegExp(`(${VARIABLE}|\\d+) (>>|<<) (.*)`)
export const magnitude2 = new RegExp(`(${VARIABLE}|\\d+) (>|<)(\\d+)(?:>|<) (.*)`)
export const bitshift = new RegExp(`(${VARIABLE}|\\d+) \\.(<<|>>) (\\d+)`)
export const power = new RegExp(`(${VARIABLE}|\\d+)\\^(\\d)`)
export const xor = new RegExp(`(${VARIABLE}|\\d+) x\\| (.*)`)
export const rangeLoop = new RegExp(`for \\(jawn (${VARIABLE}) = (\\d+):(\\d+)?:?(\\d+)\\) {`)
export const print = new RegExp(`print(d|l|i|w|e)?\\(`)
// export const 
